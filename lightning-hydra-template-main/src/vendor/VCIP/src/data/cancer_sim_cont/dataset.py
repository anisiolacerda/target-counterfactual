...
from src.data.cancer_sim_cont.cancer_simulation import TUMOUR_DEATH_THRESHOLD
from src.data.cancer_sim_cont.cancer_simulation import generate_params, get_scaling_params, simulate_factual, \
    simulate_counterfactual_1_step, simulate_counterfactuals_treatment_seq, simulate_output_after_actions
...
class SyntheticCancerDataset(Dataset):
    ...
    def __init__(self,
        ...
        if mode == 'factual':
            self.data = simulate_factual(self.params, seq_length, random=random)
    ...
    def process_data(self, scaling_params):
        ...
        if not self.processed:
            ...
            elif self.treatment_mode == 'continuous':
                self.data['prev_treatments'] = treatments[:, :-1, :]
                self.data['current_treatments'] = treatments
...
class SyntheticCancerDatasetCollectionCont(SyntheticDatasetCollection):
    """
    ...
    def __init__(self,
        ...
        Args:
            ...
            treatment_mode: multiclass / multilabel
        """
        super(SyntheticCancerDatasetCollectionCont, self).__init__()
        self.seed = seed
        np.random.seed(seed)
        ...
        self.test_f = SyntheticCancerDataset(chemo_coeff, radio_coeff, num_patients['test'], window_size, max_seq_length, 'test',
                                            lag=lag, treatment_mode=treatment_mode, random=True)
