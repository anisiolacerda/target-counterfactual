...
def calc_diameter(volume):
    volume = max(volume, 0.)  # avoid negative volumes
...
def simulate_factual(simulation_params, seq_length, assigned_actions=None, random=False):
    ...
    for i in tqdm(range(num_patients), total=num_patients):
        ...
        for t in range(1, seq_length - 1):
            ...
            radio_application_point[i, t] = scipy_beta.rvs(radio_alpha, radio_beta)
            chemo_application_point[i, t] = scipy_beta.rvs(chemo_alpha, chemo_beta)
            ...
            # print("Current chemo dose: ", current_chemo_dose)
            # print('radio application point: ', radio_application_point[i, t])
            # exit()
...
def simulate_output_after_actions(Ht, actions, scaling_params):
    ...
    factual_cancer_volume = Ht['cancer_volume'].cpu().numpy()
    factual_unscaled_outputs = Ht['unscaled_outputs'].cpu().numpy()
    factual_chemo_application = Ht['chemo_application'].cpu().numpy()
    factual_radio_application = Ht['radio_application'].cpu().numpy()
    patient_types = Ht['patient_types'].cpu().numpy()
    
    # Get parameters for each patient
    alphas = Ht['alpha'].cpu().numpy()
    betas = Ht['beta'].cpu().numpy()
    beta_cs = Ht['beta_c'].cpu().numpy()
    rhos = Ht['rho'].cpu().numpy()
    Ks = Ht['K'].cpu().numpy()
    ...
    for i in range(num_patients):
        ...
        # Initialize counterfactual trajectories
