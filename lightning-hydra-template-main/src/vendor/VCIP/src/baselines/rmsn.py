...
from src.baselines.time_varying_model import TimeVaryingCausalModel
from src.baselines.utils import grad_reverse, BRTreatmentOutcomeHead, AlphaRise, clip_normalize_stabilized_weights
from src.baselines.utils_lstm import VariationalLSTM
    ...
    def _init_specific(self, sub_args: DictConfig, encoder_r_size: int = None):
        ...
        try:
            ...
            self.density_head = nn.Sequential(
                nn.Linear(self.seq_hidden_units, 2),
                nn.Softplus()  
            )
...
class RMSNPropensityNetworkTreatment(RMSN):
    ...
    def forward(self, batch):
        ...
        if self.hparams.dataset.treatment_mode != 'continuous':
            treatment_pred = self.output_layer(x)
            return treatment_pred
        else:
            alpha_beta = self.density_head(x)
            return alpha_beta
    ...
    def training_step(self, batch, batch_ind):
        ...
        if self.hparams.dataset.treatment_mode != 'continuous':
            ...
            alpha, beta = treatment_pred.chunk(2, dim=-1)
            beta_dist = Beta(alpha, beta)
            curr_treatments = batch['current_treatments'].clamp(min=1e-6, max=1-1e-6)
            # print(f"curr_treatments[0]:{curr_treatments[0]}, beta_dist.log_prob(curr_treatments):{beta_dist.log_prob(curr_treatments)}")
            lambda_entropy = self.config.exp.lambda_entropy
            bce_loss = -beta_dist.log_prob(curr_treatments).sum(dim=-1).mean() - beta_dist.entropy().mean() * lambda_entropy
    ...
    def predict_step(self, batch, batch_ind, dataset_idx=None):
        if self.hparams.dataset.treatment_mode != 'continuous':
            ...
            alpha_beta = self(batch)
            alpha, beta = alpha_beta.chunk(2, dim=-1)
            beta_dist = Beta(alpha, beta)
            curr_treatments = batch['current_treatments']
            prob = torch.exp(beta_dist.log_prob(curr_treatments)).clamp(min=1e-1, max=self.config.exp.max)
            return prob.cpu()
