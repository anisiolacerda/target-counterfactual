...
from src.baselines.time_varying_model import BRCausalModel
from src.baselines.utils import grad_reverse, BRTreatmentOutcomeHead, AlphaRise
...
class CRNEncoder(CRN):
    ...
    def forward(self, batch, detach_treatment=False):
        ...
        # print(f"shape of br: {br.shape}, shape of curr_treatments: {curr_treatments.shape}")
...
class CRNDecoder(CRN):
    ...
    def forward(self, batch, detach_treatment=False):
        prev_treatments = batch['prev_treatments'].float()
        prev_outputs = batch['prev_outputs'].float()
        static_features = batch['static_features'].float()
        curr_treatments = batch['current_treatments'].float()
        init_states = batch['init_state'].float()

        # print(f"prev_treatments: {prev_treatments.dtype}")
        # print(f"prev_outputs: {prev_outputs.dtype}")
        # print(f"static_features: {static_features.dtype}")
        # print(f"curr_treatments: {curr_treatments.dtype}")
        # print(f"init_states: {init_states.dtype}")
