...
def bce(treatment_pred, current_treatments, mode, weights=None):
    if mode == 'multiclass':
        ...
        return F.mse_loss(treatment_pred, current_treatments, reduce=False).mean(dim=-1)
    else:
...
class BRTreatmentOutcomeHead(nn.Module):
    ...
    def __init__(self, seq_hidden_units, br_size, fc_hidden_units, dim_treatments, dim_outcome, alpha=0.0, update_alpha=True,
        ...
        self.linear8 = nn.Linear(self.br_size, fc_hidden_units)
        self.elu5 = nn.ELU()
        self.linear9 = nn.Linear(fc_hidden_units, self.dim_treatments)

        self.treatment_head_params = ['linear2', 'linear3', 'linear8', 'linear9']
    ...
    def build_outcome(self, br, current_treatment):
        ...
        # x = x.float()
        # current_treatment = current_treatment.float()

        # print("x dtype:", x.dtype)
        # print("linear4 weight dtype:", self.linear4.weight.dtype)
        # print("linear4 bias dtype:", self.linear4.bias.dtype)
        # exit()

        # print(f'dtype of x: {x.dtype}')
        # print(f'br: {br.dtype}')
        # print(f'current_treatment: {current_treatment.dtype}')
        x = self.elu3(self.linear4(x))
        outcome = self.linear5(x)
        return outcome

    def build_br(self, seq_output):
        seq_output = seq_output.type_as(self.linear1.weight)
        br = self.elu1(self.linear1(seq_output))
        return br

    def build_RE(self, br):
        # x = x.float()
        x = self.elu5(self.linear8(br))
        RE = self.linear9(x)
        return RE
