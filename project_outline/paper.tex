...
RA-constrained selection also shifts predicted treatment patterns: constrained selections have substantially lower vasopressor rates (0.01--0.29 vs.\ 0.29--0.79 under ELBO), suggesting the constraint preferentially removes aggressive treatment plans that risk hypotension. This aligns with recent clinical evidence favoring conservative vasopressor management in hemodynamically stable patients. The 99--100\% in-target rate means nearly all selected plans keep blood pressure within the normotensive range, while the reduced vasopressor selection indicates fewer aggressive interventions---exactly the combination a clinician would seek. While we cannot verify ground-truth safety on real data (counterfactual trajectories are unobservable), the treatment pattern shift is clinically plausible and demonstrates practical value: the RA filter produces plans that are both effective and conservative.
...
\vspace{-0.8em}
\paragraph{Toward clinical guardrails.} The post-hoc filtering paradigm we propose is an instance of \emph{guardrails} for AI-driven treatment planning, analogous to shielding in safe RL~\citep{alshiekh2018shielding}: rather than training the model to be safe, we add an external safety check on its outputs. This separation has practical deployment advantages: safety thresholds can be adjusted by clinicians without retraining the model, and the same base model can operate under different constraint profiles for different patient populations or clinical contexts.

\vspace{-0.8em}
\paragraph{Limitations and future work.} (1)~$\cT$ and $\cS$ require domain expertise (Table~\ref{tab:target_size} shows robustness). (2)~On MIMIC-III, counterfactuals are unobservable. (3)~Feasibility drops at long horizons. (4)~Sequential ignorability is assumed. Future work includes formal sensitivity analysis, multi-outcome targets, and adaptive threshold selection. Despite these limitations, reach-avoid constrained selection demonstrates that meaningful safety improvements in counterfactual planning can be achieved through principled post-hoc filtering, without any modification to the underlying generative model.
